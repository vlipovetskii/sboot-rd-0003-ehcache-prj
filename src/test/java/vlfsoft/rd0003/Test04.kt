...
@TestPropertySource(properties = [
    ...
    "context.initializer.classes=vlfsoft.rd0003.Test04.AppBeansInitializer"
])
class Test04 : KBeanFactoryA.Holder {
    ...
    /**
     * Cons: Kotlin Implementation by Delegation https://kotlinlang.org/docs/reference/delegation.html#implementation-by-delegation
     * can't be leveraged to avoid boilerplate delegation code
     * Pros: [Cacheable] is on class implementation method.
     * Pros: It's possible to switch between cacheable and not cacheable implementations easily.
     * bean { DataQryApi(DataQryApi.NotCacheable()) } <-> bean bean { DataQryApi.NotCacheable() }
     */
    class DataQryApi(override val notCacheable: DataQryApiA.NotCacheable) :
            DataQryApiA,
            DataQryApiA.NotCacheable.Holder {

        override val findInvokeCount get () = notCacheable.findInvokeCount

        @Cacheable(cacheNames = ["DataQryApiA4"]) //, key = "#id")
        override fun find(id: Int) = notCacheable.find(id)
